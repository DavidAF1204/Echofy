...
/**
 * Represents a ReEchoBox component.
 *
 * @component
 ...
 * @param {nunmber} props.reechoID - The function to handle post click.
 * @returns {React.ReactElement} A ReEchoBox element.
 */
function ReEchoBox({ showPostBox, handlePostClick, reechoID }) {
  ...
  /**
   * Sending request posting ReEcho data to backend. Clear the data in
   * {useState} post text, post image and file input when the request is HTTP-200
   *
   * @api reEcho
   * @returns null.
   */
  const handleReEchoSubmit = () => {
    ...
    Promise.all(imagePromises)
      .then(async (imageDataArray) => {
        ...
        const reEcho = await fetch("http://localhost:3001/reEcho", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  return (
    <Modal show={showPostBox} onHide={handlePostClick} centered>
      {/* 
                ...
                The Modal head contains the ReEcho title,
      ...
      <Modal.Header closeButton>
        <Modal.Title>ReEcho</Modal.Title>
      </Modal.Header>
      ...
      <Modal.Footer>
        ...
        <Button variant="outline-danger" onClick={handleReEchoSubmit}>
          Post
        </Button>
      </Modal.Footer>
    </Modal>
